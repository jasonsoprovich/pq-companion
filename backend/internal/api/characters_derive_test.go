...
func TestDeriveBlock_Base(t *testing.T) {
	...
	b := h.deriveBlock(osui(), aa, 0, 100, statBlock{}, 0, nil)
...
func TestDeriveBlock_Compounding(t *testing.T) {
	...
	b := h.deriveBlock(osui(), aa, 0, 100, item, 0, buffs)
...
func TestDeriveBlock_BuffStaRaisesHP(t *testing.T) {
	h := &charactersHandler{}
	base := h.deriveBlock(osui(), db.AABonuses{}, 0, 100, statBlock{}, 0, nil)
	// A buff granting only +100 STA (no flat HP).
	buffed := h.deriveBlock(osui(), db.AABonuses{}, 0, 100, statBlock{}, 0,
