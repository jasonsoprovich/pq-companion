...
- **Live Maps** — Rendered zone maps parsed directly from your EQ client files, with Outline and Detailed display modes, live player position tracking, a pop-out overlay window, and user-authored annotations that write back to the in-game `/map`. NPC spawn points show with patrol routes and trap markers, and a user-installed Brewall-style map pack can be rendered as a third display mode.
...
- **Lockouts Tracker** — Live `/sll` countdowns for loot and legacy lockouts, tracked per character so you always know when an instance or flag is available again, plus a per-zone Loot Lockouts sub-tab showing every character's status for that zone at a glance.
...
- **Progression Recap** — Rolling 7/30/90-day level, AA, spell, and skill progression for any character, or ranked most-active-first across all of them, with total coin/change and a scoped loot panel. A new tab on Character Info.
