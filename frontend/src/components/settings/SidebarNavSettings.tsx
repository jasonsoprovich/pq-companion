...
export default function SidebarNavSettings(): React.ReactElement {
  ...
  async function persist(nextOrder: SecOrder, nextHidden: Set<string>) {
    if (!cfg) return
    setSaving(true)
    const flatOrder = NAV_SECTIONS.flatMap((s) => nextOrder[s.id] ?? [])
    const next: Config = {
      ...cfg,
      preferences: {
        ...cfg.preferences,
  ...
  function resetDefaults() {
    const next: SecOrder = {}
    NAV_SECTIONS.forEach((s) => { next[s.id] = s.items.map((i) => i.to) })
    setSecOrder(next)
    setHidden(new Set())
    persist(next, new Set())
  }

  const visibleCount = NAV_SECTIONS.reduce((n, s) => n + s.items.filter((i) => !hidden.has(i.to)).length, 0)
      ...
      <section className="rounded-lg p-4" style={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
        ...
        <div className="flex flex-col gap-4">
          {NAV_SECTIONS.map((section) => {
            const keys = secOrder[section.id] ?? section.items.map((i) => i.to)
            return (
              <div key={section.id}>
