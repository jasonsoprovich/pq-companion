...
import { getItemShoppingRoute } from '../services/api'
import type {
  ItemShoppingRoute,
  ItemShoppingStop,
  ShoppingListItem,
...
interface Props {
  // The selected items to plan for. Selection is owned by the caller (e.g. a
  // recipe's component list) — this panel just renders the route for what
  // it's given.
  itemIds: number[]
  // Remove an item from the run; the caller drops it from the selection and
  // re-passes a shorter itemIds, which re-plans the route.
  onRemoveItem: (id: number) => void
  onClose: () => void
}

// Common starting points offered in the dropdown, in addition to whatever
// zones the route itself visits. Mostly neutral hubs plus the major
// hometowns and tradeskill-relevant zones. Mirrors ShoppingRoutePanel's
// START_HUBS (spell version) — same hubs make sense for buying components.
...
const EMPTY_ROUTE: ItemShoppingRoute = {
  ...
  excluded_by_zone: [], candidate_zones: [], total_zones: 0, total_items: 0,
}
...
// groupVendors assigns each item at a stop to exactly one vendor so the buy
// list is unambiguous ("grab these from this merchant"). Vendors are tried in
// order of how many of the needed items they carry (most first), so the
...
// Any item not claimed by a vendor (shouldn't happen, but be safe) is returned
// as leftover rather than silently dropped.
function groupVendors(stop: ItemShoppingStop): {
  groups: { vendor: ItemShoppingVendor; items: ShoppingListItem[] }[]
  leftover: ShoppingListItem[]
} {
  const byId = new Map(stop.items.map((s) => [s.id, s]))
  const ordered = [...stop.vendors].sort(
    (a, b) =>
      b.item_ids.length - a.item_ids.length ||
      formatNPCName(a.name).localeCompare(formatNPCName(b.name)),
  )
  const assigned = new Set<number>()
  const groups: { vendor: ItemShoppingVendor; items: ShoppingListItem[] }[] = []
  for (const v of ordered) {
    ...
    if (items.length > 0) {
      ...
      groups.push({ vendor: v, items })
    }
  }
  const leftover = stop.items.filter((s) => !assigned.has(s.id))
  return { groups, leftover }
}

// dropItemFromRoute removes one item from an already-solved route in place,
// without re-solving. This keeps the itinerary frozen as the player shops:
// checking an item off drops it (and any vendor/stop it empties) but never
// reshuffles which towns the rest of the run visits. Re-optimizing is an
// explicit action (the Re-plan button).
function dropItemFromRoute(route: ItemShoppingRoute, id: number): ItemShoppingRoute {
  const stops = route.stops
    .map((s) => ({
      ...s,
      items: s.items.filter((it) => it.id !== id),
      vendors: s.vendors
        .map((v) => ({ ...v, item_ids: v.item_ids.filter((x) => x !== id) }))
        .filter((v) => v.item_ids.length > 0),
    }))
  ...
  return {
    ...
    total_items: stops.reduce((n, s) => n + s.items.length, 0),
  }
}

// StopCard renders one zone in the itinerary.
function StopCard({
  stop, index, onExcludeItem, onExcludeZone,
}: {
  stop: ItemShoppingStop
  index: number
  onExcludeItem: (s: ShoppingListItem) => void
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        {stop.reason === 'anchor' && (
          <span
            ...
            title="This zone is the only source of at least one item, so the trip is mandatory."
          >
            only source
          </span>
        )}
        ...
        <button
          ...
          title="Skip this town — re-route its items to the next-best source"
        >
          <Ban size={13} />
        </button>
      </div>

      {/* Vendors, each with the items to grab from them as children. The
      ...
      <div className="mt-2 flex flex-col gap-2">
        ...
        {/* Defensive: items with no resolvable vendor at this stop. */}
        {leftover.length > 0 && (
...
export default function ItemShoppingRoutePanel({ itemIds, onRemoveItem, onClose }: Props): React.ReactElement {
  ...
  const [route, setRoute] = useState<ItemShoppingRoute | null>(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  // The route is frozen once solved: checking items off updates it locally
  // (see removeItem) but never re-solves. `replan` is bumped to force a fresh
  ...
  // explicit Re-plan. Checking items off (the itemIds prop shrinking) does
  // NOT re-solve; removeItem updates the frozen route locally instead. The
  // fetch always reads the current itemIds, so Re-plan optimizes for whatever
  ...
  // now stale vs. a fresh solve.
  const removeItem = (id: number) => {
    setRoute((r) => (r ? dropItemFromRoute(r, id) : r))
    setStale(true)
    onRemoveItem(id)
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          {route && !loading && (
            <span className="text-xs" style={{ color: 'var(--color-muted-foreground)' }}>
              {route.total_items} {route.total_items === 1 ? 'item' : 'items'} ·{' '}
        ...
        >
          ...
          {/* Re-plan: re-solve for the items still on the list. Highlighted
              once the frozen route has gone stale from checking items off. */}
          <button
            ...
            title="Re-optimize the route for the items you still need"
          >
            <RefreshCw size={11} />
            Re-plan
          </button>
        ...
        <div className="flex flex-1 flex-col gap-2 overflow-y-auto px-5 py-4">
          ...
          {!loading && !error && route && route.stops.length === 0 && (
            <p className="py-8 text-center text-sm" style={{ color: 'var(--color-muted)' }}>
              {itemIds.length === 0
                ? 'No components selected — pick some ingredients to plan a run.'
                : 'No vendor route found for these items.'}
            </p>
          )}

          {/* The route stays put as you shop; checked-off items just drop
          ...
          {!loading && !error && stale && route && route.stops.length > 0 && (
            ...
            >
              ...
              Route is frozen while you shop — checked-off items drop out but the
          ...
          {!loading && !error && route && route.stops.map((stop, i) => (
            <StopCard
              ...
              stop={stop}
              index={i}
              onExcludeItem={(s) => removeItem(s.id)}
              onExcludeZone={toggleZone}
            />
          ))}

          {/* Items whose only towns were filtered out by alignment */}
          ...
          {/* Items whose only towns are ones the player chose to skip */}
          ...
          {/* Items whose only source is a zone that isn't released yet (PoK) */}
          ...
          {/* Items no vendor sells anywhere */}
          {!loading && !error && route && (route.unavailable?.length ?? 0) > 0 && (
            ...
            >
              ...
              <p className="mt-1.5 text-[11px]" style={{ color: 'var(--color-muted)' }}>
                These come from drops, tradeskills, or foraging — check each item's
                sources for details.
              </p>
            </div>
          )}

          {/* Removing an item here unchecks it in the caller; re-add it there. */}
        </div>
      </div>
    </div>
  )
