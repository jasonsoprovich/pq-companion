        ...
        <Section title="Effects">
          ...
          {item.click_effect > 0 && item.max_charges > 0 && (
            <StatRow
              label="Charges"
              value={
                isLikelyRechargeable(item.max_charges, item.click_effect, item.item_type)
                  ? `${item.max_charges} (rechargeable)`
                  : item.max_charges
              }
            />
          )}
...
export default function ItemDetailModal({ item, open, onClose }: ItemDetailModalProps): React.ReactElement | null {
  ...
  useEffect(() => {
    ...
    getItemQuests(item.id)
      .then(setQuests)
      .catch(() => setQuests({ walkthrough: [], used_in: [] }))
